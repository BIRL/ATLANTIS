...
% Last Modified by GUIDE v2.5 29-Dec-2017 11:42:00
...
elseif simulationTime < 1
    ...
    return;
elseif simulationTime > 50000 && simulationTime < 10000000
    choice = questdlg('Specified simulation time is very large. Please note this can take long time. Do you want to continue ?', ...
    'Warning', 'Yes', 'No', 'No');
    switch choice
        case 'Yes'
...
elseif simulationTime > 10000000
...
errorMargin = str2num(get(handles.EditErrorMargin, 'String'));
if isempty(errorMargin) || size(errorMargin, 1) > 1 || size(errorMargin, 2) > 1
    uiwait(errordlg('Please enter error margin value.', 'Missing or Invalid input.'));
    return; 
elseif errorMargin < 0
    uiwait(errordlg('Error margin cannot be negative. Please enter an positive value.', 'Error'));
    return;
elseif errorMargin > 0  && errorMargin < 10e-10
    choice = questdlg('Specified error margin value is very strigent. The simulation may take long time to reach steady states. Do you want to continue?', ...
    'Warning', 'Yes', 'No', 'No');
    switch choice
        case 'Yes'
            myNetwork.ErrorThreshold = errorMargin;
        otherwise
            uiwait(msgbox('Please update error margin value to proceed.', 'Warning'));
            return;
    end
elseif errorMargin > 10e-4
    uiwait(errordlg('Invalid error margin value. Please enter a value between 0 and 10e-4.', 'Error'));
    return;
else
    myNetwork.ErrorThreshold = errorMargin;
end
...
% ADD A CHECK IF THE USER HAS CHECK HERIUSTIC PA. IF SO THEN CHANGE
% ASSSOCIATE THE CELL FATES WITH PRUNNNED STATE SPACE
