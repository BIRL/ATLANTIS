...
function [errormsg, finishTime] = generate_AttractorLandscape
...
try
    if strcmp(mappingMethod, 'sammon')
        ...
        if ~isempty(myNetwork.Trajectory)
            ...
            steadyStates = vertcat(steadyStates, statesInTrajectory);
        end
        ...
        if ~isempty(myNetwork.SteadyStateProbabilities)
            ...
            steadyStates = vertcat(steadyStates, stateSpace(ind, :));
        end
...
% disp(['Time taken to process the state space: ', num2str(toc(totalTimE))]);
...
try
    ...
    if ~mappingPerformed
        ...
        if strcmp(mappingMethod, 'sammon')
            ...
            disp(['Time Taken for Sammon Mapping: ', num2str(toc)]);
            x = xy(:,1);
            y = xy(:,2);
...
if plotPEL
    ...
    drawnow
    disp(['Time Taken for Plotting Potential Energy Landscape: ', num2str(toc)]);
end
...
if plotPL
    ...
    drawnow
    disp(['Time Taken for Plotting Probability Landscape: ', num2str(toc)]);
end
...
if plotBSL
    ...
    drawnow;
    disp(['Time Taken for Plotting Basin Size Landscape: ', num2str(toc)]);
end
...
finishTime = toc(totalTimE);

disp(['Total Time Taken for Plotting : ',  num2str(finishTime )]);
