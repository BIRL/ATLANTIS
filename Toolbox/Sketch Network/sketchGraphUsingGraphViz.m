% Copyright (c) 2009, Bjorn Wielens
% All rights reserved.

function [bool, figHandle] = sketchGraphUsingGraphViz(dotFile,  varargin)

myNetwork = get(0, 'UserData');

try
    progressbar('Sketching Network. Please wait...');
        
    % GET SKETCHING OPTIONS
    sketchingOptions = getappdata(0,  'SketchingOptions');
    
    % NEW PATH
    resultsPath = myNetwork.ResultFolderName;
    rpos = strfind(myNetwork.ResultFolderName, 'Results');
    newFolder = [resultsPath(1:rpos+7), 'Temp', filesep];
    [~, ~] = mkdir(newFolder);
    
    % COPY DOT FILE
    fileToBeCopied = fullfile(dotFile{2},  dotFile{1});
    [status,  ~,  ~] = copyfile(fileToBeCopied,  newFolder);
    
    % GIVE ERROR IF FILE CANNOT BE COPIED
    if status  ==  0
        undefined/0;
    end
    ...
    if strcmp(sketchingOptions.Engine, 'Circos')
        engine = 'circo';
    elseif strcmp(sketchingOptions.Engine, 'Dot')
        engine = 'dot';
    else
        engine = matlabEngine;
    end
    ...
    dotFileName = [newFolder dotFile{1}];
    
    if ismac
        engine = ['/usr/local/Cellar/graphviz/2.40.1/bin/', engine];
    end
    
    seP = ' -Tplain ';
    
    [status, returnCode] = system([engine seP '"' dotFileName '"']);
    ...
    if status ~= 0 && exist('matlabEngine', 'var')
        graphVizAbsent = 1;
        [status, returnCode] = system([matlabEngine seP dotFileName]);
    end
...
%     set(figHandle,  'Resize', 'off');
    ...
    % START SKETCHING
    fileContent = regexp(returnCode, ['\', 'n'], 'split')';
    graphLine = fileContent{~cellfun('isempty', regexp(fileContent, '^graph'))};
    tokens = regexp(graphLine,['\', 's+'],'split');
    scale = str2double(tokens{2});
    
    if strcmp(sketchingOptions.FigureBackground, 'Black')
        sketchingOptions.FigureBackground = [0 0 0];
    elseif strcmp(sketchingOptions.FigureBackground, 'Grey')
        sketchingOptions.FigureBackground = [0.752941 0.752941 0.752941];
    else
        sketchingOptions.FigureBackground = [1 1 1];
    end
    
    % UPDATE FIGURE PROPERTIES
    fig1 = gcf;
    set(fig1,  'Color',  sketchingOptions.FigureBackground,  'Resize', 'on');
    set(axisHandle, 'xTick',  [], 'YTick',  [],  'XColor',  sketchingOptions.FigureBackground,  'YColor',  sketchingOptions.FigureBackground,  'Color',  sketchingOptions.FigureBackground);
    ...
    tokens=regexp(nodeLines,...
           ['node','\','s+(?<nodeID>"[^"]+"|[','\','w','\','d]+)','\','s+(?<x>[','\','d','\','.','\','+e]+)','\','s+(?<y>[','\','d','\','.','\','+e]+)','\','s+(?<w>[','\','d','\','.','\','+e]+)','\','s+(?<h>[','\','d','\','.','\','+e]+)','\','s+(?<label>".*"|[','\','w','\','d]+)','\','s+(?<style>','\','w+)','\','s+(?<shape>[','\','w','\','d]+)','\','s+(?<edgeColour>(?:[#','\','w][','\','w','\','d]+)|[','\','d','\','.]+','\','s+[','\','d','\','.]+','\','s+[','\','d','\','.]+)','\','s+(?<bgColour>(?:[#','\','w][','\','w','\','d]+)|[','\','d','\','.]+','\','s+[','\','d','\','.]+','\','s+[','\','d','\','.]+)'],'tokens');
    tokens=[tokens{:}]';
    tokens = [tokens{:}]';
    ...
    if fullMode
        ...
        txtcolor = cellfun(@(x) VisibleTextColor(x),  bgColours,  'UniformOutput',  false);
        textObj = text(x, y, labels, 'HorizontalAlignment', 'center', 'Interpreter', 'none', 'verticalAlignment', 'middle', 'margin', 0.1, 'FontWeight', 'bold');
        for k = 1:size(txtcolor,  1)
            set(textObj(k), 'Color', txtcolor{k});
        end
    else
    ...
    [~,tailLocation]=ismember(tailNodes,nodeIDs);
    tailCoord=[x(tailLocation)';y(tailLocation)'];
    tipNodes=regexprep(tokens(:,2),'"','');
    [~,tipLocation]=ismember(tipNodes,nodeIDs);
    ...
    for i = 1:size(splinePts, 1)
        extrAEle = mod(numel(splinePts{i}), numSpline{i});
        vec = splinePts{i};
        splinePts{i} = vec(1:end-extrAEle);
    end
    ...
    [~, ~, splinePts]=cellfun(@bezier,num2cell(zeros(numel(splinePts),1)),num2cell(ones(numel(splinePts),1)),splinePts,num2cell(space,2),'Uniformoutput',false);
    ...
    % ADD LABELS
%     if sketchingOptions.ShowLabels
    labels=tokens(:,5);
    labels=labels(~cellfun('isempty',labels));
    if ~isempty(labels)
        labels=regexp(labels,['(["]','\','s?','\','W?','\','d+','\','s?["])','\','s+([','\','d','\','.]+)','\','s+([','\','d','\','.]+)'],'tokens');
        labels=[labels{:}]';
        labels=[labels{:}]';
        ...
        if any(~empty)
            ...
            labelXoffset = 0.01;
            labelYoffset = 0;
            labelColor = [0.6 0.5 0.4];
            labeltext = text(labelX(~empty).*(1-labelXoffset),labelY(~empty).*(1-labelYoffset),labelText(~empty),'HorizontalAlignment','center','verticalAlignment','middle','interpreter','none','Color',labelColor);
            FontSizes =  get(textObj,'fontSize');
            FontModifier = 0.9;
            set(labeltext, 'fontSize', FontSizes{1}*FontModifier, 'Visible', 'off');
        end
        
        menuLabel = ['Show ', 'Edge Labels'];
        addContextMenuToFigure(rootMenu, menuLabel, {@toggleEdgeLabels, labeltext});
    end
%     end
    
    % ADD OPTION TO ADD AND REMOVE LABELS
    ...
    if fullMode
        ...
        for i = 1:size(patchObj,  1)
            Vertices_X{i} = patchObj{i}.Vertices(:, 1);
            Vertices_Y{i} = patchObj{i}.Vertices(:, 2);
        end
        
        tipPolyX = Vertices_X(tipLocation)';
        tipPolyY = Vertices_Y(tipLocation)';
        
        tailPolyX = Vertices_X(tailLocation)';
        tailPolyY = Vertices_Y(tailLocation)';
    else
    ...
    if ~isempty(regexp(arrowStyle, '>', 'once'))
        fwdArrow = true;
    elseif ~isempty(regexp(arrowStyle, '<', 'once'))
        revArrow = true;
    else
        btArrow = true;
    end
    ...
    if fullMode
        maxFont = max(cell2mat(get(textObj, 'fontSize')));
        set(textObj, 'FontUnits', 'normalized');
        cellfun(@resizeText,num2cell(textObj),num2cell(patchObj));
%         for k = 1:size(txtcolor,  1)
%             uistack(textObj(k), 'top');
%         end
        fontSizes = get(textObj, 'fontSize');
        setappdata(figureHandle, 'fontSizes', cell2mat(fontSizes));
        setappdata(axisHandle, 'originalX', get(axisHandle, 'Xlim'));
        setappdata(axisHandle, 'originalY', get(axisHandle, 'Ylim'));
        setappdata(figureHandle, 'patches', patchObj);
        setappdata(figureHandle, 'texts', textObj);
    end
    
    % ADD TITLE
    figHandle.Name = sketchingOptions.PlotTitle;
    figHandle.NumberTitle = 'off';
    title(sketchingOptions.PlotTitle, 'Color', 1-sketchingOptions.FigureBackground, 'FontSize', maxFont*1.6, 'HorizontalAlignment', 'center');
    ...
    if fullMode
        ...
        setAllowAxesRotate(rotateHandle, axisHandle, false);
    ...
    le(2) = plot(NaN,NaN,'-g');
    le(3) = plot(NaN,NaN,'-b');
    legendsObj = legend(le,{'Inhibition','Activation','Undirected'}, 'location', 'BestOutside');
...
%     cd(oldFolder);
    ...
    if graphVizAbsent
        enableDisableFig(findall(0, 'tag','GraphVizMenu'), 'off');
        enableDisableFig(findall(0, 'tag','ATLANTIS_Main'), 'off');
        uiwait(errordlg(['Graphviz was not found installed on your system. NOTE: The matlab default engine (mwdot) was used for sketching. The plot will may contain artifacts. Using graphviz is recommended.' ...
            ' SOLUTIONS: Add graphViz bin path to the system enviornment variables PATH. Try restarting MATLAB. (See documentation for more help.)'], 'Warning'));
        enableDisableFig(findall(0, 'tag','GraphVizMenu'), 'on');
    end
...
function vec = VisibleTextColor(bgColor)
...
% labeltext = getappdata(0,  'labeltext');
...
% labelfontsize = get(labeltext, 'fontSize');
% set(labeltext,  'fontSize', labelfontsize{1}*(newScale/3));
% setappdata(0,  'labeltext',  labeltext);
...
set(thisText, 'FontSize', 0.02);
fontSize = 0.02;

patchX = {};
patchY = {};

for i = 1:size(thisPatch,  1)
    patchX{i} = thisPatch{i}.Vertices(:, 1);
    patchY{i} = thisPatch{i}.Vertices(:, 2);
end

patchX = cell2mat(patchX);
patchY = cell2mat(patchY);
...
while all(textOK)
    ...
    set(thisText, 'FontSize', fontSize*1.1);
    textExtent = get(thisText, 'extent');
    textX = [textExtent(1), textExtent(1) + textExtent(3)];
    textY = [textExtent(2), textExtent(2) + textExtent(4)];
    textOK = inpolygon(textX, textY, patchX, patchY);
...
ScaleDownFactor = 0.4;
if noFill
    patch((drawOut(1, :).*ScaleDownFactor + tipX), (drawOut(2, :).*ScaleDownFactor + tipY), 'FaceColor', [1 1 1], 'EdgeColor', colour);
else
    patch((drawOut(1, :).*ScaleDownFactor + tipX), (drawOut(2, :).*ScaleDownFactor + tipY), colour, 'edgecolor', colour);
end
...
factoR = 1;
if ~isempty(property)
    patchHandle = patch(patchX, patchY, [0 0 0], 'FaceColor', bgColour, 'EdgeColor', edgeColour, property, value, 'lineWidth', extraOutlines + factoR);
else
    patchHandle = patch(patchX, patchY, [0 0 0], 'FaceColor', bgColour, 'EdgeColor', edgeColour, 'lineWidth', extraOutlines + factoR);
end
...
colours.lightgray = [1 1 1];
colours.lightgrey = [1 1 1];
