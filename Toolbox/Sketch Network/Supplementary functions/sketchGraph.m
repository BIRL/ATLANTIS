...
function [bool, figHandle] = sketchGraph(dotFile,  varargin)
...
try
    ...
    % NEW PATH
    newFolder = 'Sketch Network\Supplementary functions\';
    ...
    [status, returnCode] = system([engine ' -Tplain ' dotFile{1}]);
    ...
    if status ~= 0 && exist('matlabEngine', 'var')
        graphVizAbsent = 1;
        [status, returnCode] = system([matlabEngine ' -Tplain ' dotFile{1}]);
    end
    ...
    if sketchingOptions.ShowLabels
        ...
        if ~isempty(labels)
            ...
            if any(~empty)
                ...
                FontModifier = 0.9;
                set(labeltext, 'fontSize', FontSizes{1}*FontModifier);
            end
        end
    end
...
%         end
    ...
    if graphVizAbsent
        errordlg(['Graphviz was not found installed on your system. NOTE: The matlab default engine (mwdot) was used for sketching. The plot will may contain artifacts. Using graphviz is recommended.' ...
